...
*LAlt:: {
    ...
    Integer currentID := WinExist("A")
    ; MsgBox(currentID . " " . WinGetProcessName(currentID) . " " . WinGetClass(currentID)) ; DEBUG

    if currentID {

        String name := WinGetProcessName(currentID)
        String class := WinGetClass(currentID)
    ...
    ; open goto window
    if (gotoID and gotoID != currentID) { ; if the goto is the same as current, then current overrides it because i wouldnt usually do this unless i wanna hide it. i already tested for this before, but i just wanna be safe.
    ...
    ; hide current first (its usually the most incriminating)
    if currentID {

        hideWindow(hiddenWindows, currentID)
